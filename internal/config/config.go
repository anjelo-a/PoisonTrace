...
// LoadReadOnlyAPIFromEnv loads config for read-only API serving.
// It preserves all non-Helius invariants while allowing omitted Helius credentials.
func LoadReadOnlyAPIFromEnv() (Config, error) {
	cfg, err := LoadFromEnv()
	if err == nil {
		return cfg, nil
	}

	cfg, baseErr := loadBaseConfigFromEnv()
	if baseErr != nil {
...
func loadBaseConfigFromEnv() (Config, error) {
	maxWalletsPerRun, err := getEnvInt("MAX_WALLETS_PER_RUN", 25)
	if err != nil {
		return Config{}, err
	}
...
func (c Config) ValidateReadOnlyAPI() error {
	if c.DatabaseURL == "" {
		return errors.New("DATABASE_URL is required")
	}
	if _, err := url.ParseRequestURI(c.DatabaseURL); err != nil {
